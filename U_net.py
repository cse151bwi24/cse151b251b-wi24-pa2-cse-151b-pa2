...
class U_net(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.relu(self.conv4(self.relu(self.conv3(self.maxpool(x1)))))
        x3 = self.relu(self.conv6(self.relu(self.conv5(self.maxpool(x2)))))
        x4 = self.relu(self.conv8(self.relu(self.conv7(self.maxpool(x3)))))
        x5 = self.relu(self.conv10(self.relu(self.conv9(self.maxpool(x4)))))

        # Decoder
        x6 = self.deconv1(x5)
        crop1 = transforms.CenterCrop((x6.shape[2], x6.shape[3]))
        x6 = self.relu(self.conv12(self.relu(self.conv11(torch.concatenate((crop1(x4), x6), dim=1)))))

        x7 = self.deconv2(x6)
        crop2 = transforms.CenterCrop((x7.shape[2], x7.shape[3]))
        x7 = self.relu(self.conv14(self.relu(self.conv13(torch.concatenate((crop2(x3), x7), dim=1)))))

        x8 = self.deconv3(x7)
        crop3 = transforms.CenterCrop((x8.shape[2], x8.shape[3]))
        x8 = self.relu(self.conv16(self.relu(self.conv15(torch.concatenate((crop3(x2), x8), dim=1)))))

        x9 = self.deconv4(x8)
        crop4 = transforms.CenterCrop((x9.shape[2], x9.shape[3]))
        x9 = self.relu(self.conv18(self.relu(self.conv17(torch.concatenate((crop4(x1), x9), dim=1)))))
